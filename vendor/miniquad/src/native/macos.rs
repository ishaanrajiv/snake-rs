...
unsafe fn create_opengl_view(
    ...
    let mut swap_interval = requested_swap_interval;
    ...
    F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
    ...
    let view = match conf.platform.apple_gfx_api {
        AppleGfxApi::OpenGl => create_opengl_view(
            &mut display,
            conf.sample_count,
            conf.high_dpi,
    ...
    let current_runloop = msg_send_![class!(NSRunLoop), currentRunLoop];
    let timer_interval = if conf.platform.swap_interval == Some(0) {
        0.001 // keep wakeups frequent when uncapped
    } else {
        0.016 // ~60Hz
    };
    let timer = match conf.platform.apple_gfx_api {
        AppleGfxApi::OpenGl => msg_send_![class!(NSTimer), timerWithTimeInterval:timer_interval
                                                           target:view
                                                           selector:sel!(setNeedsDisplayHack)
                                                           userInfo:nil
                                                           repeats:YES],
        AppleGfxApi::Metal => msg_send_![class!(NSTimer), timerWithTimeInterval:timer_interval
                                                          target:view
                                                          selector:sel!(draw)
                                                          userInfo:nil
                                                          repeats:YES],
